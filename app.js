...
//task 9 create rout to add product in cart
app.post('/cart/add',async(req,res)=>{
    try{
        ...
        //checking if user already have any cart
        let cart;
        if(user.cart){
            ...
            //if product is not already in cart add it to cart
...
//task 10 create rout delete product in cart
app.delete('/cart/product/delete',async(req,res)=>{
    try{
        ...
        if(!cart){
         ...
         await cart.save();
         return res.status(200).json({
            message:"product removed successfilly",
            cart:cart
         })
    }catch(error){
        console.log(error);
